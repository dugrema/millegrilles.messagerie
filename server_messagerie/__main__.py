...
from server_messagerie.WebServerMessagerie import WebServerMessagerie
from server_messagerie.Commandes import CommandMessagerieHandler

logger = logging.getLogger(__name__)

LOGGING_NAMES = ['server_messagerie']
...
class CollectionsAppMain(WebAppMain):
    ...
    def init_command_handler(self) -> CommandMessagerieHandler:
        return CommandMessagerieHandler(self)

    async def configurer(self):
        await super().configurer()

    async def configurer_web_server(self):
        self._web_server = WebServerMessagerie(self.etat, self._commandes_handler)
...
async def demarrer():
    ...
    logger.info("Run main messagerie")
    await main_inst.run()
    logger.info("Fin main messagerie")
